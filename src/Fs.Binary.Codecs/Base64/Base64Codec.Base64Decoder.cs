            ...
            public ConvertStatus ConvertData ( ReadOnlySpan<char> inputData, int inputIndex, int inputCount, Span<byte> outputData, int outputIndex, int outputCount, bool flush, out int inputUsed, out int outputUsed )
            {
                ...
                while (true)
                {
                    switch (_currentState)
                    {
                        ...
                        case State.Reading:
                            while ((_decodedCount < 4) && (inputIndex < inputEnd))
                            {
                            ...
                            if (_decodedCount == 4) goto case State.BeginWriting;
                        ...
                        case State.Writing:
                            while ((_decodedCount < 7) && (outputIndex < outputEnd))
                            {
                                outputData[outputIndex++] = (byte)((_decodedBits & 0xFF0000) >> 16);
                                outputUsed++;
                                _decodedBits <<= 8;
                                _decodedCount++;
                            }

                            if (_decodedCount == 7) goto case State.BeginReading;
                        ...
                        case State.Flushing:
                            {
                                bool invalidQuantum = (_decodedCount == 1);
                                if (invalidQuantum)
                                {
                            ...
                            _decodedBits <<= (int)(24u - (6u * _decodedCount));
                            _decodedCount = 7u - (_decodedCount - 1);
